<!-- ====== HEADER ====== -->
<p align="center">
  <img src="./banner.png?v=1782531225" width="100%" />
</p>

<h1 align="center">iOS Developer (｡•̀ᴗ-)✧</h1>

<p align="center">
  <a href="https://www.linkedin.com/in/ruslan-melanin-803589278/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?logo=linkedin&logoColor=white" />
  </a>
  <a href="https://drive.google.com/file/d/12S7MZeUkmC8cNIedGhHOibDid1tObPKP/view?usp=sharing" target="_blank">
    <img src="https://img.shields.io/badge/CV-PDF-ff4d6d" />
  </a>
  <a href="https://t.me/animepusya" target="_blank">
    <img src="https://img.shields.io/badge/Telegram-26A5E4?logo=telegram&logoColor=white" />
  </a>
</p>

<p align="center">
  <a href="mailto:rus.melanin@outlook.com">
    <img src="https://img.shields.io/badge/Email-rus.melanin@outlook.com-6b7280?logo=gmail&logoColor=white" />
  </a>
</p>

---

## ✦ About

iOS developer building real apps from idea to release.

I care about how features behave in real usage: architecture, state management, UX details and performance.

---

## ✦ Tech Stack

<p>
  <img src="https://img.shields.io/badge/Swift-F05138?logo=swift&logoColor=white" />
  <img src="https://img.shields.io/badge/Git-E74C3C?logo=git&logoColor=white" />

  <img src="https://img.shields.io/badge/UIKit-FF8E53" />
  <img src="https://img.shields.io/badge/PhotosUI-F9A826" />

  <img src="https://img.shields.io/badge/SwiftData-FFD93D" />

  <img src="https://img.shields.io/badge/MVVM-6BCB77" />
  <img src="https://img.shields.io/badge/WidgetKit-00C9A7" />
  <img src="https://img.shields.io/badge/StoreKit%202-2ECC71" />

  <img src="https://img.shields.io/badge/EventKit-0081CF" />
  <img src="https://img.shields.io/badge/Clean%20Architecture-4D96FF" />

  <img src="https://img.shields.io/badge/Combine-6C5CE7" />
  <img src="https://img.shields.io/badge/UserNotifications-845EC2" />
  <img src="https://img.shields.io/badge/Swift%20Concurrency-A66CFF" />

  <img src="https://img.shields.io/badge/SwiftUI-FF6B6B" />
  <img src="https://img.shields.io/badge/Charts-FF6F91" />

  <img src="https://img.shields.io/badge/Foundation%20Models-Apple%20AI-black" />
</p>

---

## ✦ Current Project

### Mind Widgets (In Development)

Modern iOS 17+ application focused on personal widgets, phrase collections, and on-device AI features.

Key features:
- Apple Intelligence integration using Foundation Models
- WidgetKit with App Group data sharing
- SwiftData persistence layer
- JSON export/import
- Custom widget rendering and image processing
- Feature-based MVVM architecture
- Dependency Injection and protocol-oriented design
- Local notifications and widget synchronization

Tech:
Swift, SwiftUI, SwiftData, Foundation Models, WidgetKit, App Groups, CoreGraphics, UserNotifications

---

## ✦ Main Released Project

### Task Planner

Task Planner is a productivity app with recurring tasks, calendar planning, statistics, widgets, reminders, Apple Calendar integration, and subscriptions.

Available on the App Store.
<p>
  <a href="https://apps.apple.com/us/app/task-planner-daily-schedule/id6762282029" target="_blank">
    <img src="https://img.shields.io/badge/App%20Store-0D96F6?logo=appstore&logoColor=white" />
  </a>
</p>

Tech:
Swift, SwiftUI, SwiftData, MVVM, Clean Architecture, Combine, Swift Concurrency, UserNotifications, EventKit, WidgetKit, StoreKit 2, PhotosUI, Charts, Git

---

## ✦ Featured Projects

| Project | What it is | Tech | Link |
|---|---|---|---|
| **Mind Widgets** | App that helps users keep important thoughts, ideas, and reminders visible through customizable Home Screen widgets. | SwiftUI, SwiftData, MVVM, Foundation Models, WidgetKit, CoreGraphics | Private Development |
| **Task Planner** | Productivity app with recurring tasks, statistics, widgets, Apple Calendar sync, reminders, photo attachments, and subscriptions | SwiftUI, SwiftData, MVVM, Clean Architecture, WidgetKit, EventKit, StoreKit 2 | [![App Store](https://img.shields.io/badge/App%20Store-0D96F6?logo=appstore&logoColor=white)](https://apps.apple.com/us/app/task-planner-daily-schedule/id6762282029) [![Repo](https://img.shields.io/badge/Open-Repo-black?logo=github)](https://github.com/animepusya/Task-Planner-iOS) |
| **Danetka. Think Again.** | Logic-based riddle card game with remote content, offline cache, localization | SwiftUI, MVVM, JSON API | [![App Store](https://img.shields.io/badge/App%20Store-0D96F6?logo=appstore&logoColor=white)](https://apps.apple.com/us/app/danetka/id6764052533) [![Repo](https://img.shields.io/badge/Open-Repo-black?logo=github)](https://github.com/animepusya/Yes-No-Game) |
| **MovieQuiz** | Quiz app with API-driven questions and MVP architecture | UIKit, MVP, REST API | [![Repo](https://img.shields.io/badge/Open-Repo-black?logo=github)](https://github.com/animepusya/MovieQuiz) |
